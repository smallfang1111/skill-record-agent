...
def build_system() -> str:
    ...
    "When a user mentions a skill, ALWAYS use todo_write to create a task list for it.\n"
...
"""
    # 这里调用 s09 的 memory 写入函数
    # 如果你用的是原版 code.py，它已经有 memory 系统了
    # 可以直接使用 write_memory() 函数
...
def update_progress(skill: str, completed: str, note: str = "") -> str:
    ...
    for todo in CURRENT_TODOS:
        if todo["content"] == completed:
            todo["status"] = "completed"
            break
    
    # 存入记忆
    progress_note = f"{skill} 完成：{completed}"
    if note:
     ...
     "input_schema": {"type": "object", "properties": {"description": {"type": "string"}}, "required": ["description"]}},
     {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "steps": {"type": "array", "items": {"type": "string"}, "description": "学习步骤列表"}
            },
